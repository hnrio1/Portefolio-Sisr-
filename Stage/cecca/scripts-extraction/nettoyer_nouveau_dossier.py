...
FICHIER_ENTREE = r"C:\Users\henri\Desktop\extraction_nouveau_dossier.csv"
FICHIER_SORTIE = r"C:\Users\henri\Desktop\extraction_nouveau_dossier_FINAL.xlsx"
...
def main():
    ...
    df = pd.read_csv(FICHIER_ENTREE, sep=';', encoding='utf-8-sig')

    print(f"Nombre de lignes avant nettoyage: {len(df)}")

    col_signature = 'Date Signature'
    col_debut = 'Debut Exercice'
    col_fin = 'Fin Exercice'

    # Convertir en string
    df[col_signature] = df[col_signature].astype(str)
    df[col_debut] = df[col_debut].astype(str)
    df[col_fin] = df[col_fin].astype(str)

    # Compter les "Non trouvé"
    non_trouve_signature = df[col_signature].str.contains('Non trouvé', na=False).sum()
    ...
    print(f"  - 'Non trouvé' dans signature: {non_trouve_signature}")
    print(f"  - 'Non trouvé' dans début: {non_trouve_debut}")
    print(f"  - 'Non trouvé' dans fin: {non_trouve_fin}")

    # Supprimer les lignes avec "Non trouvé" dans les dates d'exercice
    lignes_avant = len(df)
    ...
    # Convertir TOUTES les dates au format JJ/MM/AAAA
    df_clean[col_signature] = df_clean[col_signature].apply(convertir_date)
